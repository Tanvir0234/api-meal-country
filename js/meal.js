const searchFood = ()=>{
    const searchInput = document.getElementById('search-field');
    const searchText = searchInput.value;
    if(searchText==''){
        alert ('please input which you want!!!');

    }
    else{
        const url = `https://www.themealdb.com/api/json/v1/1/search.php?s=${searchText}`
        fetch(url)
        .then(res => res.json())
        .then(data => searchMealResult(data.meals));
        searchInput.value = '';
    }
   
    
}

const searchMealResult = (meals) => {
const serachResult = document.getElementById('search-result');

//------clear previous product----------
serachResult.innerHTML ='';
//serachResult.textContent='';
/*
if(meals.length == 0){
    console.log('no found');
}
*/
meals.forEach(meal =>{
//console.log(meal);

const div = document.createElement('div');
div.classList.add('col')
div.innerHTML = `

<div onclick="loadDetailMeal('${meal.idMeal}')"  class="card">
            <img src="${meal.strMealThumb}" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">${meal.strMeal}</h5>
              <p class="card-text">${meal.strInstructions.slice(0,200)}</p>
            </div>
          </div>

`
serachResult.appendChild(div);

})

}

const loadDetailMeal = mealId =>{
    const url = `https://www.themealdb.com/api/json/v1/1/lookup.php?i=${mealId}`

     fetch(url)
    .then (res => res.json())
    .then (data => displayMealdeatail(data.meals[0]));

}

const displayMealdeatail = meal => {
//console.log(meal);
const mealDetail = document.getElementById('load-deatil');
const div = document.createElement('div');
div.classList.add("card");
div.innerHTML = `
<img src="${meal.strMealThumb}" class="card-img-top" alt="...">
<div class="card-body">
  <h5 class="card-title">${meal.strMeal}</h5>
  <p class="card-text">${meal.strInstructions.slice(0,150)}</p>
  <a href="${meal.strYoutube}" class="btn btn-primary">See YouTube</a>
</div>
`
mealDetail.appendChild(div);
}
